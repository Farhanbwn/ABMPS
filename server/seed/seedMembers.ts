...
export async function seedMembers(options: { disconnectOnComplete?: boolean } = {}) {
  ...
  try {
    ...
    if (mongoose.connection.readyState === 0) {
      console.log(`[Seed Members] Connecting to MongoDB: ${mongoUri.replace(/:([^@]+)@/, ':****@')}`);
    ...
    console.log(`[Seed Members] Loading members from: ${jsonFilePath}`);
    ...
    console.log(`[Seed Members] Found ${rawMembers.length} member records in JSON. Resetting member collections...`);
    ...
    console.log(`[Seed Members] Successfully inserted ${insertedMembers.length} members.`);
    ...
    if (Array.isArray(rawRenewals) && rawRenewals.length > 0) {
      ...
      if (validRenewalDocs.length > 0) {
        ...
        console.log(`[Seed Members] Successfully inserted ${renewalsInserted} renewal records.`);
      }
    }
    ...
    console.log(`\n========================================`);
    console.log(`[Seed Members] COMPLETED SUCCESSFULLY`);
    console.log(`========================================`);
    ...
    console.error('[Seed Members] Error seeding members:', error);
    if (disconnectOnComplete) {
      process.exit(1);
    }
    throw error;
...
if (require.main === module) {
  seedMembers({ disconnectOnComplete: true })
    .then(() => process.exit(0))
